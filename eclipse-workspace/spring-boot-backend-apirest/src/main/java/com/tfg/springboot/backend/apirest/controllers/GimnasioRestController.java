package com.tfg.springboot.backend.apirest.controllers;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

import com.tfg.springboot.backend.apirest.models.entity.Gimnasio;
import com.tfg.springboot.backend.apirest.models.entity.Gimnasio;
import com.tfg.springboot.backend.apirest.models.services.IGimnasioService;

@CrossOrigin
@RestController
@RequestMapping("/api")
public class GimnasioRestController {

		@Autowired
		private IGimnasioService gimnasioService;
		
		@GetMapping("/gimnasios")
		public List<Gimnasio> index(){
			return gimnasioService.findAll();
		}
		
		@GetMapping("/gimnasios/{id}")
		public Gimnasio show(@PathVariable Integer id) {
			return gimnasioService.findById(id);
		}
		
		@PostMapping("/gimnasios")
		@ResponseStatus(HttpStatus.CREATED)
		public Gimnasio create(@RequestBody Gimnasio gimnasio) {
			return gimnasioService.save(gimnasio);
		}
		
		@PutMapping("/gimnasios/{id}")
		@ResponseStatus(HttpStatus.CREATED)
		public Gimnasio update(@RequestBody Gimnasio gimnasio, @PathVariable Integer id) {
			
			Gimnasio gimnasioActual = gimnasioService.findById(id);
			
			gimnasioActual.setNombre(gimnasio.getNombre());
			gimnasioActual.setDireccion(gimnasio.getDireccion());
			gimnasioActual.setN_identificacion_fiscal(gimnasio.getN_identificacion_fiscal());
			gimnasioActual.setTelefono(gimnasio.getTelefono());
			
			return gimnasioService.save(gimnasioActual);
			
		}
		
		@DeleteMapping("/gimnasios/{id}")
		@ResponseStatus(HttpStatus.NO_CONTENT)
		public void delete(@PathVariable Integer id) {
			gimnasioService.delete(id);
		}
}
